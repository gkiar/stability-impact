...
severely hamper our understanding of brain function in brain-imaging studies. However, it also highlights the potential
of leveraging this variance in estimates of brain connectivity to increase the reliability of datasets and reduce bias.
This paper demonstrates that stability evaluations are necessary as a core component of typical analytical workflows.}
% add explicit and hopeful reference to increased reliability
% make middle discussion section more explicit in terms of the generality and steps that scientists must take
%----------------------------------------------------------------------------------------
...
subjects (Hypothesis 1) is an essential prerequisite for the application of brain imaging towards identifying
individual differences~\cite{Dubois2016-yr}. In teseting hypothesis 1, we observe that the dataset is separable
...
each case ($p < 0.001$ for all), significantly higher than without instrumentation ($p < 0.005$ for all). This result
impactfully suggests the utility of perturbation methods for synthesizing robust and reliable individual estimates of
connectivity, serving as a cost effective and context-agnostic method for dataset augmentation.
...
\subsection*{Uncertainty in Brain-Behaviour Relationships}
...
stability of these analyses, we modelled the relationship between Body Mass Index (BMI) and brain
...
Importantly, this finding does not suggest that modelling brain-behaviour relationships is not possible, but rather it
...
\subsubsection*{Evaluating A Brain-Behaviour Analysis}
Though each of the above approaches explores the instability of derived connectomes and their features, many modern
studies employ modeling or machine-learning approaches, for instance to learn brain-behavior relationships or identify
...
simulations have been bundled and made available through The Canadian Open Neuroscience Platform
(\url{https://portal.conp.ca/search}, search term "Kiar"). Execution records for the image analysis can be found on
Zenodo (\url{todo})

\subsection*{Code Availability}
All software developed for processing or evaluation is publicly available on GitHub at
\url{https://github.com/gkiar/stability}. Experiments were launched using Boutiques~\cite{Glatard2018-tu} and
Clowdr~\cite{Kiar2019-sr} in Compute Canada's HPC cluster environment. A set of MCA instrumented software containers is
available on Github at \url{https://github.com/gkiar/fuzzy}. 
