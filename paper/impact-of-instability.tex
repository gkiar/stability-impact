...
pipelines~\cite{Garyfallidis2014-ql,Garyfallidis2012-gg} which were instrumented with MCA, mimicking computational noise at either the
inputs or throughout the pipelines~\cite{Denis2016-wo,Kiar2020-lb}. The pipelines were sampled $20$ times per
collection and once without perturbations, resulting in a total of $4,200$ connectomes.
...
the trustworthiness of derived connectomes for these analyses. While both pipelines show similar performance, the
probabilistic pipeline is more stable in the face of Pipeline Perturbations whereas the deterministic is more stable
under Input Perturbations ($p < 0.0001$ for all; exploratory). An evaluation of the stability of graph correlations can
in \sref{supsec:correlation}.
